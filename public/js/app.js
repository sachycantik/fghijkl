...
function showDeleteConfirmation({
  ...
  message = "Email dan seluruh lampirannya akan dihapus permanen. Tindakan ini tidak dapat dibatalkan.",
  ...
  document.getElementById("deleteConfirmAccept").textContent = confirmLabel;
  deleteConfirmReturnFocus = document.activeElement;
  modal.classList.remove("hidden");
  document.body.classList.add("nebula-dialog-open");
...
function closeDeleteConfirmation(confirmed) {
  ...
  document.body.classList.remove("nebula-dialog-open");

  const resolve = deleteConfirmResolver;
  deleteConfirmResolver = null;
  resolve?.(confirmed);

  if (deleteConfirmReturnFocus?.focus) {
...
function handleDeleteConfirmKeydown(event) {
  ...
  const buttons = [
    document.getElementById("deleteConfirmCancel"),
    document.getElementById("deleteConfirmAccept")
  ];
...
async function clearAllEmails() {
  const confirmed = await showDeleteConfirmation({
    ...
    message: "Semua email dan lampiran dalam inbox ini akan dihapus permanen. Tindakan ini tidak dapat dibatalkan.",
...
function bindEvents() {
  ...
  document.getElementById("deleteConfirmCancel").addEventListener("click", () => closeDeleteConfirmation(false));
  document.getElementById("deleteConfirmAccept").addEventListener("click", () => closeDeleteConfirmation(true));
  document.getElementById("deleteConfirmBackdrop").addEventListener("click", () => closeDeleteConfirmation(false));
  document.getElementById("deleteConfirmModal").addEventListener("keydown", handleDeleteConfirmKeydown);
